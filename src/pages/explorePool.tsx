                ...
                <tbody>
                    {
                        pools.map((item, index) => (
                            ...
                            <td>
                                <Row>
                                <span><B1>{item.id.substring(0,6)}...{item.id.substring(38,42)}</B1> <Link link={'https://arbiscan.io/address/'+item.id} target='_blank' rel="noreferrer"> <i className='fa fa-external-link' aria-hidden="true"></i> </Link> </span>
                                </Row>
                                <Badge></Badge>
                            </td>
                            <td>{item.token0.symbol}-{item.token1.symbol}</td>
                            ...
                            <td> 
                                <Row>
                                <span> <b>{item.token0.symbol}</b>={parseFloat(item.volumeToken0).toFixed(5)} </span> 
                                </Row>
                                <span><b>{item.token1.symbol}</b> ={parseFloat(item.volumeToken1).toFixed(5)}</span>
                            </td>
                            <td> 
                                <Row>
                                <span> <b>{item.token0.symbol}</b>={parseFloat(item.reserve0).toFixed(5)} </span> 
                                </Row>
                                <span><b>{item.token1.symbol}</b> ={parseFloat(item.reserve1).toFixed(5)}</span>
                            </td>
